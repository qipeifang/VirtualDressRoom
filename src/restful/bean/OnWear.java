...
@Entity
@Table(name = "T_OnWear")
@NamedQueries({ @NamedQuery(name = "OnWear.findAll", query = "SELECT onWear FROM OnWear onWear"),
		@NamedQuery(name = "OnWear.findById", query = "SELECT onWear FROM OnWear onWear WHERE onWear.id = :id") })

public class OnWear implements Cloneable, Serializable {
	...
	@AttributeOverrides({ @AttributeOverride(name = "userName", column = @Column(name = "userName")),
			@AttributeOverride(name = "dressId", column = @Column(name = "dressId")) })
...
//	// 以下两个属性不进行持久化
	 ...
	 * @throws Exception
	 */
	public OnWear(User user, Dress dress) throws Exception {
